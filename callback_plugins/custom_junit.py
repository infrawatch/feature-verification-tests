...
class CallbackModule(JunitCallbackModule):
    ...
    def __init__(self):
        ...
        # Update this to parse these values from the config file,
        # as well as the env.
        ...
        # Needed because we use "ignore_errors" on the
        # playbooks so that all the tests are run.
    ...
    def _finish_task(self, status, result):
        ...
        # Debugging
        if self._debug and task_data.name.startswith(
                self._test_case_prefix):
            print(
                f"This task ({task_data.name}) starts with "
                f"the test_prefix({self._test_case_prefix})"
            )
        if self._debug and self._test_case_prefix in task_data.name:
            print(
                f"This task ({task_data.name}) should be "
                f"reported because it contains "
                f"test_prefix({self._test_case_prefix})"
            )
        if self._debug and status == 'failed':
            print(
                f"This task ({task_data.name}) failed, "
                f"but may not be reported"
            )

    def mutate_task_name(self, task_name):
        # Debugging
        if self._debug and self._test_case_prefix not in task_name:
            print(
                "task_name (%s) does not contain prefix (%s)"
                % (task_name, self._test_case_prefix)
            )
        new_name = task_name
        # Only use the first line, so we can include IDs
        # and additional description.
        new_name = new_name.split("\n")[0]
        # This covers when a task is included, but the
        # including task is the one that is the test.
        # Only provide the last part of the name when the
        # role name is included.
        new_name = new_name.split(":")[-1]

        if len(self._test_case_prefix) > 0:
            # this one may not be needed...
            # Remove the test prefix and everything before it
            new_name = new_name.split(
                self._test_case_prefix
            )[-1]
    ...
    def _build_test_case(self, task_data, host_data):
        """This is used in generate_report.

        The task_data and host data will get passed.
        """
