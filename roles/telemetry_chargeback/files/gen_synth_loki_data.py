...
def generate_loki_data(
    ...
    for log_type_config in log_types_config:
        ...
        # "type" is log-type identifier (dict key) and output value
        type_key = log_type_config.get("type")
        if not type_key:
            logger.error("Each log type must have a 'type' field")
        ...
        if missing:
            ...
            raise ValueError(
                f"Missing required fields in {type_key!r}: {missing}"
            )
    ...
    # --- Step 5: Validate that the output is valid JSON ---
    try:
        with output_path.open('r') as f_in:
            json.load(f_in)
        logger.info("Output file validated as valid JSON.")
    except json.JSONDecodeError as e:
        logger.error(
            f"Output file is not valid JSON: {e}. "
